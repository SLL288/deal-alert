...
## GitHub Pages
- Point Pages to deploy from `/public` (or repo root if you prefer) so `index.html` and the JSON files are served.
- Ensure CI workflow commits `public/*.json` after each run.
...
## Launch checklist
- Enable GitHub Pages to serve from `/public` and note the URL (typically `https://<org>.github.io/<repo>/`).
